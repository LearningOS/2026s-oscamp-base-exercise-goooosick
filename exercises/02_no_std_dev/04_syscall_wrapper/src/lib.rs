...
pub fn sys_write(fd: usize, buf: &[u8]) -> isize {
    unsafe { syscall3(NATIVE_SYS_WRITE, fd, buf.as_ptr() as _, buf.len()) }
}

/// Read data from file descriptor `fd` into `buf`.
pub fn sys_read(fd: usize, buf: &mut [u8]) -> isize {
    unsafe { syscall3(NATIVE_SYS_READ, fd, buf.as_ptr() as _, buf.len()) }
}
...
pub fn sys_exit(code: i32) -> ! {
    unsafe {
        syscall3(NATIVE_SYS_EXIT, code as _, 0, 0);
    }
    loop {}
}
