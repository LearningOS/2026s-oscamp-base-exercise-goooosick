...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        let mut root_ppn = self.root_ppn;
        for level in [2, 1, 0] {
            let node = self.nodes.get_mut(&root_ppn).unwrap();
            let vpn = Self::extract_vpn(va, level);
            let pte = node.entries[vpn];
            if level > 0 {
                if pte & PTE_V == 0 {
                    let new_ppn = self.alloc_node();
                    self.nodes.get_mut(&root_ppn).unwrap().entries[vpn] =
                        (new_ppn << PPN_SHIFT) | PTE_V;
                    root_ppn = new_ppn;
                } else {
                    root_ppn = pte >> PPN_SHIFT;
                }
            } else {
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        let mut root_ppn = self.root_ppn;
        for level in [2, 1, 0] {
            let node = self.nodes.get(&root_ppn).unwrap();
            let vpn = Self::extract_vpn(va, level);
            let pte = node.entries[vpn];
            if pte & PTE_V == 0 {
            ...
            if pte & (PTE_R | PTE_W | PTE_X) != 0 {
                let pa = (pte >> PPN_SHIFT) << 12 | (va & ((1 << (12 + level * 9)) - 1));
