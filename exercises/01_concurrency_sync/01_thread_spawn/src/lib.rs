...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    thread::spawn(move || numbers.into_iter().map(|n| n * 2).collect::<Vec<_>>())
        .join()
        .unwrap()
}
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    thread::scope(|s| {
        (
            s.spawn(|| a.into_iter().sum()).join().unwrap(),
            s.spawn(|| b.into_iter().sum()).join().unwrap(),
        )
    })
}
...
pub fn handle_panic(value: i32, should_panic: bool) -> Result<i32, ()> {
    ...
    thread::spawn(move || if should_panic { panic!("oops") } else { value })
        .join()
        .map_err(|_| ())
}
